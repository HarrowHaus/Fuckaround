...
def check_riff(label, r, bpm):
    ...
    print(f"   [{label}] " + genome_riff(g, bpm, "sec").tab().split("\n")[-1])
...
for sec in DOC["sections"]:
    ...
    if not r:
        print("   (no riff)")
        continue
    check_riff("A", r, sec["bpm"])
    for letter, pr in sorted(sec.get("phrase_riffs", {}).items()):
        check_riff(letter, pr, sec["bpm"])
    ...
    print(f"   drums: {d.get('mode', 'book')} "
          ...
          f"cym={d.get('prefer_cym')}  lead={lead}")
